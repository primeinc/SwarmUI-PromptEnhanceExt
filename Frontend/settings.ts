...
class PromptEnhanceSettings {
    ...
    buildModal(): HTMLElement {
        ...
        let body = field('pe_base_url', 'Base URL', 'text', 'OpenAI-compatible server. A root URL or one ending in /v1 both work. No API key is sent, so the server must not require authentication.',
